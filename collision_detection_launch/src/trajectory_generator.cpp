...
using GenerateTrajectory = collision_detection_msgs::srv::GenerateTrajectory;
TrajectoryGenerator::TrajectoryGenerator()
    : Node("trajectory_generator")
{
    service_ =
        this->create_service<GenerateTrajectory>("generate_trajectory_srv",
                                                 std::bind(&TrajectoryGenerator::generate_trajectory_service, this, std::placeholders::_1, std::placeholders::_2));

    service1_ =
        this->create_service<GenerateTrajectory>("generate_trajectory_srv1",
                                                 std::bind(&TrajectoryGenerator::generate_trajectory_service, this, std::placeholders::_1, std::placeholders::_2));
}

moveit::planning_interface::MoveGroupInterfacePtr TrajectoryGenerator::setupMoveGroup(std::string planning_group)
{
    return std::make_shared<moveit::planning_interface::MoveGroupInterface>(shared_from_this(), planning_group);
}
